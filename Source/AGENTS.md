...
- Advance the recorded `phnt` sync point only after processing the upstream commits in order, with no unreviewed in-scope `phnt` changes before it.
...
- Prioritize correctness and staying current with the selected source of truth (Windows SDK/WDK or `phnt`) over backward compatibility with older local definitions; do not add compatibility aliases, legacy typedefs, fallback macro names, or other compatibility shims unless they are present in the selected source of truth or explicitly requested.
- Do not replace a better local definition with an upstream fallback form; evaluate correctness, usability, type information, and this repository's header structure before synchronizing.
...
- Stop and ask before synchronizing any uncertain item.
- Stop and ask when an uncertainty blocks progress or requires a project decision.
- Stop and ask when an upstream `phnt` change appears questionable.
