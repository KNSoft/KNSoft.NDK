...
typedef struct HSTRING_HEADER
{
    union
    {
        struct
        {
            ...
            ULONG Length; // UTF-16 code units, excluding the trailing null.
            ULONG Padding[2];
            PCWSTR Buffer;
        };
        union
...
// String creation APIs represent empty strings as NULL. Strings are immutable and may contain embedded nulls.
...
_STATIC_ASSERT(sizeof(HSTRING_HEADER) == sizeof(MS_HSTRING_HEADER));
_STATIC_ASSERT(__alignof(HSTRING_HEADER) == __alignof(MS_HSTRING_HEADER));
